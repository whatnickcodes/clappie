...
export function create(ctx) {
  ...
  const onSave = ctx.data?.onSave || 'stay';

  ctx.setTitle('Editor');
  ctx.setDescription('Make changes');
  ...
  function render() {
    ...
    view.add(ButtonFullWidth({
      label: 'Save',
      onPress: () => { saveFile(); ctx.pop(); }
    }));

    // Focus: textarea when editing, save button when not
    ...
    if (isEditing) {
      ...
      view.focusIndex = 1;  // Focus save button
    }
